...
use axum::Router;
use clap::Parser;
use yew_ssr_v2::{ServerApp, ServerAppProps};
...
struct Opt {
    ...
    #[clap(short = 'm', long = "max_conn", default_value = "10000")]
    max_conn: usize,
}
...
async fn main() {
    ...
    // let app = Router::new()
    //     .route("/", get(render))
    //     .layer(tower_limit::ConcurrencyLimitLayer::new(opts.max_conn));
    ...
    let app = Router::new()
        .layer(tower_limit::ConcurrencyLimitLayer::new(opts.max_conn))
        // .layer(axum::AddExtensionLayer::new(opts.dir.clone()))
        // .layer(axum::AddExtensionLayer::new((index_html_before.clone(), index_html_after.clone())))
        // .layer(axum::AddExtensi::new(Duration::from_secs(1)))
